...
@router.post(
    "/{production_id}/cases/{case_id}/findings/{finding_id}/escalate",
...
def escalate_finding(
    ...
    case_id: str,
    finding_id: str,
    payload: EscalateFindingRequest,
    request: Request,
) -> Finding:
    services = _services(request)
    try:
        ...
        raise HTTPException(status_code=404, detail="Production not found") from error
    try:
        member = services.workspace_member_repository.get(payload.assignee)
    except WorkspaceMemberNotFound as error:
        raise HTTPException(status_code=422, detail="Assignee is not a workspace member") from error
    try:
        services.case_repository.update_finding_status(
            case_id, finding_id, reviewer_status=ReviewerStatus.ESCALATED
        )
        services.case_repository.update_finding_meta(
            case_id,
            finding_id,
            assignee=member.name,
            due_date=payload.due_date,
        )
        due_date = f" due {payload.due_date}" if payload.due_date else ""
        return services.case_repository.add_finding_comment(
            case_id,
            finding_id,
            FindingComment(
                id=str(uuid4()),
                author="RightsRadar",
                body=f"Escalated to {member.name} ({member.role.value}){due_date}.",
                created_at=datetime.now(UTC),
            ),
        )
    except CaseRepositoryNotFound as error:
